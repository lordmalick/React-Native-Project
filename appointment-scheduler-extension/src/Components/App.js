...
export default class App extends React.Component {
  ...
  handleFetchError(err) {
    console.log(err)
    this.setState({ snackbarMessage: 'Erreur chargement' })
  }
        ...
        <Toolbar>
          <ToolbarGroup firstChild={true}>
            <DropDownMenu
              ...
              <MenuItem value={0} primaryText="Filtrer par Date" />
              <MenuItem value={1} primaryText="List les Reuinions" />
            </DropDownMenu>
            <DatePicker
          ...
          <ToolbarGroup lastChild={true}>
            <RaisedButton
              ...
              label={`Supprimer${data.selectedRows.length ? '(' + data.selectedRows.length + ')' : ''}`} />
          </ToolbarGroup>
        </Toolbar>
        <Table
